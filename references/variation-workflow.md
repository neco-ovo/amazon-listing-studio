...
Use `scripts/studio.js record-variation-candidate --project-dir <dir> --input <candidate.json>` for an explicitly scoped `child_main` or `shared_image` candidate. Then use `scripts/studio.js approve-variation --project-dir <dir> --input <approval.json>` with exactly one of `child_main`, `shared_image`, `parent_listing`, `child_listing`, or `variation_final` as `scopeType` and `userAction: "approved"`. Child paths and IDs are never interchangeable, and fields belonging to one scope must not be reused for another. The legacy `record-candidate` and `approve` commands remain the single-product route.
