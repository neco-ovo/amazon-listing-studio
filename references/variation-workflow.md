...
Use `scripts/studio.js record-variation-candidate --project-dir <dir> --input <candidate.json>` for an explicitly scoped `child_main` or `shared_image` candidate. Recording freezes the inspected file hash and exact role; approval rehashes the saved file and rejects any byte or role change. Then use `scripts/studio.js approve-variation --project-dir <dir> --input <approval.json>` with exactly one of `child_main`, `shared_image`, `parent_listing`, `child_listing`, or `variation_final` as `scopeType` and `userAction: "approved"`. Child paths and IDs are never interchangeable, and fields belonging to one scope must not be reused for another. The legacy `record-candidate` and `approve` commands remain the single-product route.

Final approval freezes Family Identity and Parent versions, ordered theme dimensions, exact active Child tuples, every Child Product Master and effective Listing version, scoped image maps, marketplace, product type, and rule status. Finalize either a full Family or one exact Child. A Family package contains Parent copy, complete Child copy, one physical copy per shared asset, and the Variation Matrix; exact-Child delivery excludes unrelated siblings while retaining Parent identity and applicable shared assets.
