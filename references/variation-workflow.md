...
Record scoped images with `record-variation-candidate`; inspection and hash bind one byte snapshot. Approve one item with `approve-variation` or an approved set with `approve-variation-batch`. Final `approve-variation` validates current common facts and locks a draft Family Identity in the same transaction before freezing Parent, ordered theme, active tuples, Product Masters, complete Listings, asset maps, marketplace, product type, and rule status. Scope and file hashes are automatic integrity records, not a separate user confirmation.
