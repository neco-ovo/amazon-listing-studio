...
The approved main image is the first product-identity reference for every secondary. After the user approves the gallery plan and layout direction, generate the full planned secondary set and present it for one consolidated review. Repair or regenerate only rejected or directly affected images. Default roles are three distinct application scenes, one size/spec card, one material/detail card, and one back/structure card; replace any role whose required facts are unavailable.
